'use strict';
// italyadoption.js -- pure-JS helpers for the Italy adoption planner kit.
// Issue: MyZubster-Ecosystem/MyZubster-Robot #24 ("[Italy] MyZubster adoption in Italy").
const { REGULATORS, EVENTS, MARKET, PARTNERS, BETA_TESTERS, IT_LOCALE } = require('./itdata');
...
function complianceChecklist(regulators) {
  ...
  return Object.keys(table).map(code => ({
    code, name: table[code].name, items: [
      ...
      'Designate responsabile preposto',
...
function partnershipTracker(partners) {
  ...
  return {
    ...
    coldirettiStage: (list.find(p => p.id === 'coldiretti') || {}).stage,
  };
}
...
// ---------- Beta-tester pipeline (Castrocaro; 5 active robots target) ----------
...
function betaTesterPipeline(testers) {
  ...
  return {
    ...
    activeRobots: onboarded, // issue outcome = 5 active robots
    stages: BETA_STAGES,
    byStage,
...
// ---------- Italian (it) i18n coverage ----------
function itLocaleKeys(locale) {
  return Object.keys(locale || IT_LOCALE);
}
function itTranslationCoverage(keys, locale) {
  const map = locale || IT_LOCALE;
...
function marketEntryPlan(market) {
  ...
  return {
    ...
    pilotRegion: m.topRegions[0], // Emilia-Romagna (Castrocaro)
    expansionRegions: m.topRegions.slice(1, 3),
  };
}
// ---------- End-to-end Italy adoption plan ----------
function buildItalyAdoptionPlan(options) {
  ...
  return {
    ...
    itI18n: itTranslationCoverage(Object.keys(IT_LOCALE)),
    marketEntry: marketEntryPlan(market),
    gate: overallRegScore(regulators) >= 60 && betaTesterPipeline(testers).activeRobots >= 0,
  };
}
module.exports = {
  REGULATORS, EVENTS, MARKET, PARTNERS, BETA_TESTERS, IT_LOCALE, STAGES, BETA_STAGES,
